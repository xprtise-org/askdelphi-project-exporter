...
{
    public class ConceptTopicPublisher(
        ILogger<ConceptTopicPublisher> logger,
            ...
            ) : TopicPublisherBase(logger, publicationSettings, askDelphiTopicClient, askDelphiResourceClient, askDelphiRelationsClient, fs), ITopicPublisher
    {
        ...
        private readonly ILogger<ConceptTopicPublisher> logger = logger;
        ...
        public bool IsPublisherFor(IAskdelphiPublisherContext context, string topicNamespace) => "http://tempuri.org/imola-concept".Equals(topicNamespace, StringComparison.OrdinalIgnoreCase);
        ...
        public async Task<string?> PublishTopic(IAskdelphiPublisherContext context, TopicListEntryIdentifier topicListEntry, ResponseTopicVersionHistoryTableModel versionForPublicationStage)
        {
            ...
            try
            {
                ...
                IEnumerable<IEnumerable<(AskDelphi.EditingAPI.DTO.Editors.EditorValue editorValue, AskDelphi.EditingAPI.DTO.Editors.TopicPartFieldEditor editor)>> parts = PartsHelper.GetList(getPartsResponse, "content", "concept-part", "concept-part") ?? [];
                ConceptModel.ConceptStepModel[] list = await Task.WhenAll(parts.Select(async (x) =>
                {
                    string? createResourceResult = await CreateResourceFor(context, topicGuid, PartsHelper.GetFile(x, "concept-part-resource"));
                    return new ConceptModel.ConceptStepModel
                    {
                        ResourceFilename = createResourceResult,
                        Body = PartsHelper.GetString(x, "concept-part-body"),
                        Title = PartsHelper.GetString(x, "concept-part-title"),
                    };
                }));

                ConceptModel result = new()
                {
                    ...
                    Tags = tags,
                    Body = PartsHelper.GetString(getPartsResponse, "content", "body", "body"),
