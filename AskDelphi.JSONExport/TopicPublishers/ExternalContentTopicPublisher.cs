...
{
    public class ExternalContentTopicPublisher(
        ILogger<ExternalContentTopicPublisher> logger,
            ...
            ) : TopicPublisherBase(logger, publicationSettings, askDelphiTopicClient, askDelphiResourceClient, askDelphiRelationsClient, fs), ITopicPublisher
    {
        ...
        private readonly ILogger<ExternalContentTopicPublisher> logger = logger;
        ...
        public bool IsPublisherFor(IAskdelphiPublisherContext context, string topicNamespace) => "http://tempuri.org/doppio-external".Equals(topicNamespace, StringComparison.OrdinalIgnoreCase);
        ...
        public async Task<string?> PublishTopic(IAskdelphiPublisherContext context, TopicListEntryIdentifier topicListEntry, ResponseTopicVersionHistoryTableModel versionForPublicationStage)
        {
            ...
            try
            {
                ...
                ExternalContentModel result = new()
                {
                    ...
                    LinkURL = PartsHelper.GetString(getPartsResponse, "content", "link-meta-data", "url"),
                    LinkTarget = PartsHelper.GetString(getPartsResponse, "content", "link-meta-data", "open-in"),
                    LinkType = PartsHelper.GetString(getPartsResponse, "content", "link-meta-data", "link-type")
                };
